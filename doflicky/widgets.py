...
#
#  Copyright 2014-2017 Ikey Doherty <ikey.doherty@gmail.com>
#
...
class PackageLabel(Gtk.VBox):
    ...
    def __init__(self, pkg, old_pkg, interactive=False):
        ...
        label_string = "<b>{}</b> - <small>{}</small>\n{}".format(
            pkg.name, pkg.version, self.make_valid(str(pkg.summary)))
        self.label_title = Gtk.Label(label_string)
    ...
    def mark_status(self, status):
        if status == 'INSTALL':
            self.image_status.set_from_icon_name(INSTALL_ICON,
                                                 Gtk.IconSize.SMALL_TOOLBAR)
        elif status == 'UNINSTALL':
            self.image_status.set_from_icon_name(REMOVE_ICON,
                                                 Gtk.IconSize.SMALL_TOOLBAR)
        elif status == 'UPDATE':
            self.image_status.set_from_icon_name(UPDATE_ICON,
                                                 Gtk.IconSize.SMALL_TOOLBAR)
    ...
    def reset_image(self):
        if self.old_package is not None:
            ...
            icon_sz = Gtk.IconSize.SMALL_TOOLBAR
            if new_version > old_version:
                self.image_status.set_from_icon_name(OLD_ICON, icon_sz)
                self.package_status = 'UPDATE'
            else:
                self.image_status.set_from_icon_name(INSTALLED_ICON, icon_sz)
                self.package_status = 'UNINSTALL'
        else:
            self.image_status.set_from_icon_name(AVAILABLE_ICON, icon_sz)
